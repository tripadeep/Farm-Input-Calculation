...
Date d = new Date();
	...
	String query="update soil set id=?,crop_name=?,ideal_temp=?,irrigation_desc=?,ideal_ph=?,ideal_type=?,ideal_moisture=?,temperature_desc=?,moisture_desc=?,soil_type_desc=?,pH-desc=?,post_by=?,post_date=? where id ="+id ;
	PreparedStatement ps = con.prepareStatement(query);
	ps.setString(1,id);
	ps.setString(2,crop_name);
	ps.setString(3,ideal_temp);
	ps.setString(4,irrigation_desc);
	ps.setString(5,ideal_ph);
	ps.setString(6,ideal_type);
	ps.setString(7,ideal_moisture);
	ps.setString(8,temperature_desc);
	ps.setString(9,moisture_desc);
	ps.setString(10,soil_type_desc);
	ps.setString(11,pH_desc);
